...
class Train:
    ...
    def train_per_epoch(self, optimizer, lr_scheduler):
        ...
        for time_series, node_feature, label in self.train_dataloader:
            ...
            predict = self._extract_logits(model_output)
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            self.metrics["Test AUC"].append(test_result[0])
            self.metrics["Test F1"].append(test_result[-4])
            self.metrics["Test Recall"].append(test_result[-5])
            self.metrics["Test Precision"].append(test_result[-6])
            self.metrics["Test Sensitivity"].append(test_result[-1])
            self.metrics["Test Specificity"].append(test_result[-2])
            ...
            wandb.log({
                ...
                'macro F1': test_result[-4],
                'macro recall': test_result[-5],
                'macro precision': test_result[-6],
            })
        ...
        self.save_result(np.array(list(all_results.values())).T)

    def get_metrics(self):
        return {
            'Test Accuracy': self.metrics['Test Accuracy'],
            'Test AUC': self.metrics['Test AUC'],
            'Test F1': self.metrics['Test F1'],
            'Test Recall': self.metrics['Test Recall'],
            'Test Precision': self.metrics['Test Precision'],
            'Test Sensitivity': self.metrics['Test Sensitivity'],
            'Test Specificity': self.metrics['Test Specificity'],
        }
